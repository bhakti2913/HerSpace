...
export class MoodtrackerComponent implements OnInit {
  ...
  itosave:number=0;
  jtosave:number=0;
  getperioddatadate:string=1+"-"+  this.months[0]+"-"+2023;
...
getperioddata(i:number,j:number){
  ...
  this.getperioddatadate=this.daysofweek(i,j)+"-"+  this.months[this.month]+"-"+2023;
  this.getperioddatastring=res.payload.data();
  this.getperioddatastring = this.getperioddatastring["pain"];
  
  console.log(this.daysofweek(i,j)+"-"+  this.months[this.month]+"-"+2023);
})
}
  ...
  onSubmit() {
    ...
    var a = new Date(2023, this.perioddate.getMonth(), this.perioddate.getDate());
    this.temperioddate = new Date(2023, this.perioddate.getMonth(), this.perioddate.getDate());
    a.setDate(a.getDate() + 28)
    for (let index = 0; index < 3 && this.periodarray.length < 3; index++) {

      var a = new Date(2023, this.temperioddate.getMonth(), this.temperioddate.getDate());
      a.setDate(a.getDate() + 28);
  ...
  setoffset(currmonth: number) {
    if (new Date(2023, currmonth, 1).getDay()) {
      this.offsetcurrentmonth = new Date(2023, currmonth, 1).getDay() - 1;
    }
    else {
      this.offsetcurrentmonth = new Date(2023, currmonth, 1).getDay();
    }
  }
...
console.log(this.painboolean);
    ...
    var d1: any = new Date(2023, 2, 22); //"now"
    var d2: any = new Date(2023, 0, 2);  // some date
    ...
    this.offsetcurrentmonth = (new Date(2023, 1, 1).getDay());
